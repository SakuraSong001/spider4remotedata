...
if __name__ == "__main__":
    ...
    for stat in statList:
        sHref = r'https://aeronet.gsfc.nasa.gov/cgi-bin/data_display_aod_v3?site={}&nachal=2&level=1&place_code=10'.format(stat)
        ...
        if 2005 <= first <= 2012 or 2005 <= last <= 2012 or (first <= 2005 and last >= 2012):
            for year in range(max(first, 2005), min(last, 2012)+1):
                ...
                try:
                    checkbox = driver.find_element(By.NAME, downItem)
                    checkbox.click()
                except NoSuchElementException:
